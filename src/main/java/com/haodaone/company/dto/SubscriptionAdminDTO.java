...
public record SubscriptionAdminDTO(Long id, Long companyId, Plan plan, SubscriptionStatus status, Integer employeeLimit,
                                   Integer deviceLimit, LocalDate startDate, LocalDate renewalDate, BigDecimal rate, BigDecimal amount) {
    public static SubscriptionAdminDTO from(Subscription s) { return new SubscriptionAdminDTO(s.getId(), s.getCompany().getId(), s.getPlan(), s.getStatus(), s.getEmployeeLimit(), s.getDeviceLimit(), s.getStartDate(), s.getRenewalDate(), s.getRate(), s.getAmount()); }
